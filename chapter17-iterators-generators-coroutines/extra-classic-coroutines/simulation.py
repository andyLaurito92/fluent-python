...
class Taxi:
    def __init__(self, id: int, num_travels_to_perform: int) -> NoReturn:
        self.id = id
    ...
    def _drop_passanger(self) -> NoReturn:
...
class TaxiSimulation:
    def __init__(self, num_taxis: int) -> NoReturn:
        self.taxis = []
        print(f"Creating {num_taxis} taxis")
        for i in range(num_taxis):
            self.taxis.append(Taxi(i, randint(1, 10)).simulate())
    ...
    def start(self) -> NoReturn:
        ...
        while self.taxis:
            try:
                ...
                next_taxi = self.taxis[next_taxi_idx]
                next(next_taxi)
            except TaxiFinishedService as e:
                self.taxis.remove(next_taxi)
